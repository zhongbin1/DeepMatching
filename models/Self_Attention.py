    ...
    def __init__(
      ...
      embedding_size, num_hidden, num_da, num_r, num_mlp_hidden, l2_reg_lambda=0.0):
        ...
        with tf.name_scope('Self_Attention'):
            ...
            self.A_left = tf.nn.softmax(A_left, name='attention_left')
            ...
            self.A_right = tf.nn.softmax(A_right, name='attention_right')

        with tf.name_scope('Setence_Embedding'):
        ...
        with tf.name_scope('Interaction'):
            #self.Matching_matrix = tf.matmul(self.M_left, tf.transpose(self.M_right, [0, 2, 1]))
            self.Matching_matrix = tf.concat([self.M_left, self.M_right], 2)
            #self.Matching_matrix = tf.reshape(, [-1, ])
        ...
        with tf.name_scope('Penality'):
            #I_left = tf.tile(tf.eye(num_r), [self.A_left.shape[0], 1])  # tf.tile 沿维度分别扩充
            I_left = tf.reshape(tf.tile(tf.eye(num_r), [tf.shape(self.A_left)[0], 1]), [-1, num_r, num_r])

            #I_right = tf.tile(tf.eye(num_r), [self.A_right.shape[0], 1])
            I_right = tf.reshape(tf.tile(tf.eye(num_r), [tf.shape(self.A_right)[0], 1]), [-1, num_r, num_r])

            self.penality_left = tf.reduce_mean(tf.square(tf.norm(tf.matmul(self.A_left, tf.transpose(self.A_left, [0, 2, 1])) - I_left,
                                                           ord='fro', axis=[-2, -1])))
            self.penality_right = tf.reduce_mean(tf.square(tf.norm(tf.matmul(self.A_right, tf.transpose(self.A_right, [0, 2, 1])) - I_right,
                                                            ord='fro', axis=[-2, -1])))

        with tf.name_scope('Loss'):
            #softmax_cross_entropy_with_logits
            cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=self.scores, labels=self.input_y)
            self.loss = tf.reduce_mean(cross_entropy) + l2_reg_lambda*(self.penality_left + self.penality_right)
